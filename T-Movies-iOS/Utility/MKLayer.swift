...
open class MKLayer: CALayer, CAAnimationDelegate {
    ...
    open var rippleDuration: CFTimeInterval = 0.15
    open var rippleLayerColor: UIColor = .white
    ...
    open func setRippleColor(color: UIColor,
                             withRippleAlpha rippleAlpha: CGFloat = 0.15,
                             withBackgroundAlpha backgroundAlpha: CGFloat = 0) {
    ...
    open func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        guard self.isRippleActive else {
            return
        }
        self.stopEffectsImmediately()
    ...
    func forceStartEffectAtPos(location: CGPoint) {
        self.stopEffectsImmediately()
        self.startEffects(atLocation: location)
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + self.rippleDuration) {
            self.stopEffects()
        }
    }
    ...
    private func nearestInnerPoint(point: CGPoint) -> CGPoint {
        ...
        let dotx = point.x - centerX
        let doty = point.y - centerY
        let dist = sqrt(dotx * dotx + doty * doty)
        if let backgroundLayer = self.rippleLayer {
            if dist <= backgroundLayer.bounds.size.width / 2 {
                return point
            }
            let dot = backgroundLayer.bounds.size.width / 2 / dist
            let posX = centerX + dot * (point.x - centerX)
            let posY = centerY + dot * (point.y - centerY)
